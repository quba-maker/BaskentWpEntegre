...
const domain = 'ai.qubamedya.com';
const baseUrl = `https://${domain}/baskent/inbox`;
...
(async () => {
  ...
  const loginAndGetPage = async (name, logsArray) => {
    ...
    await page.goto(`https://${domain}/login`, { timeout: 60000 });
  ...
  const getSidebarBadgeCount = async (page) => {
    return await page.evaluate(() => {
      // Look for data-testid badge first, fallback to regex scanning
      ...
      const inboxLink = Array.from(document.querySelectorAll('a')).find(a => a.getAttribute('href') && a.getAttribute('href').endsWith('/inbox'));
      if (!inboxLink) return 0;
      const badges = Array.from(inboxLink.querySelectorAll('span'));
      const oldBadge = badges.find(s => /^\d+(\+)?$/.test(s.textContent.trim()));
      if (!oldBadge) return 0;
      const text = oldBadge.textContent.trim();
  ...
  const activeRow = activeId ? pageA.locator(`[data-conversation-id="${activeId}"]`) : pageA.locator('div[role="button"]').nth(activeIndex);
  ...
  let checkedActive = activeId 
    ? currentContacts.find(c => c.conversationId === activeId)
    : currentContacts[activeIndex];
  ...
  const passiveContact = freshContacts.find(c => !c.isSelected && c.unreadCount === 0);
  if (!passiveContact) {
  ...
  const passiveIndex = passiveContact.index;
  console.log(`[INFO] Selected Passive Contact: ${passiveName} (UUID: ${passiveId}, Index: ${passiveIndex})`);
  
  const initialBadge = await getSidebarBadgeCount(pageA);
  console.log(`[STATE] Sidebar badge count before unread: ${initialBadge}`);

  const passiveRow = passiveId ? pageA.locator(`[data-conversation-id="${passiveId}"]`) : pageA.locator('div[role="button"]').nth(passiveIndex);
  ...
  currentContacts = await getContactsInfo(pageA);
  let checkedPassive = passiveId 
    ? currentContacts.find(c => c.conversationId === passiveId)
    : currentContacts[passiveIndex];
  ...
  if (filteredContacts.length > 0) {
    ...
    const targetRow = targetFilteredId ? pageA.locator(`[data-conversation-id="${targetFilteredId}"]`).first() : pageA.locator('div[role="button"]').first();
    ...
    const hasDisappeared = targetFilteredId 
      ? !filteredContacts.some(c => c.conversationId === targetFilteredId)
      : !filteredContacts.some(c => c.name === targetFilteredName);
    if (hasDisappeared) {
  ...
  const targetRowPin = firstContactId ? pageA.locator(`[data-conversation-id="${firstContactId}"]`).first() : pageA.locator('div[role="button"]').first();
  ...
  let checkedContact = firstContactId 
    ? contactsInfo.find(c => c.conversationId === firstContactId)
    : contactsInfo[0];
  ...
  // Restore pin state
  const targetRowUnpin = firstContactId ? pageA.locator(`[data-conversation-id="${firstContactId}"]`).first() : pageA.locator('div[role="button"]').first();
  ...
  let restoredContact = firstContactId 
    ? contactsInfo.find(c => c.conversationId === firstContactId)
    : contactsInfo[0];
  ...
  const syncTarget = freshContactsSync.find(c => !c.isSelected && c.unreadCount === 0);
  if (!syncTarget) {
  ...
  const targetIndex = syncTarget.index;
  console.log(`[INFO] Selected sync target: ${targetName} (UUID: ${targetId}, Index: ${targetIndex})`);

  // Context A: mark unread
  const targetRowA = targetId ? pageA.locator(`[data-conversation-id="${targetId}"]`) : pageA.locator('div[role="button"]').nth(targetIndex);
  ...
  for (let attempt = 1; attempt <= 15; attempt++) {
    ...
    targetInB = targetId 
      ? currentContactsB.find(c => c.conversationId === targetId)
      : currentContactsB.find(c => c.name === targetName);
  ...
  const syncTargetRealtime = freshContactsRealtime.find(c => !c.isSelected && c.unreadCount === 0);
  ...
  const targetIndexRealtime = syncTargetRealtime.index;
  console.log(`[INFO] Selected realtime sync target: ${targetNameRealtime} (UUID: ${targetIdRealtime}, Index: ${targetIndexRealtime})`);

  // Context A: mark unread
  const targetRowRealtimeA = targetIdRealtime ? pageA.locator(`[data-conversation-id="${targetIdRealtime}"]`) : pageA.locator('div[role="button"]').nth(targetIndexRealtime);
  ...
  for (let attempt = 1; attempt <= 4; attempt++) {
    await pageB.waitForTimeout(1000);
    const currentContactsB = await getContactsInfo(pageB);
    targetInBRealtime = targetIdRealtime 
      ? currentContactsB.find(c => c.conversationId === targetIdRealtime)
      : currentContactsB.find(c => c.name === targetNameRealtime);
  ...
  // Restore read state
  const targetRowRealtimeCleanup = targetIdRealtime ? pageA.locator(`[data-conversation-id="${targetIdRealtime}"]`) : pageA.locator('div[role="button"]').nth(targetIndexRealtime);
  await targetRowRealtimeCleanup.dispatchEvent('contextmenu', { clientX: 100, clientY: 100 });
  await pageA.waitForSelector('text=Okundu yap', { timeout: 5000 });
  await pageA.click('text=Okundu yap');
  console.log("[ACTION] Page A: Clicked 'Okundu yap' to clean up.");
  await pageA.waitForTimeout(1000);

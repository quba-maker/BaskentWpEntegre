...
export default function AiControlError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  useEffect(() => {
    console.error("[AI Control Tower Error]", error);
  }, [error]);
  ...
  return (
    <div className="p-8 max-w-lg mx-auto text-center mt-20">
      ...
      <h2 className="text-lg font-bold mb-2" style={{ color: 'var(--q-text-primary)' }}>
        AI Kontrol Merkezi Yüklenemedi
      </h2>
      <p className="text-sm mb-6" style={{ color: 'var(--q-text-secondary)' }}>
        Sayfa yüklenirken bir hata oluştu. Lütfen tekrar deneyin veya sayfayı yenileyin.
      </p>

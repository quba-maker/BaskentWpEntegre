...
const EVENT_CONFIG: Record<string, { icon: any; color: string; label: string }> = {
  ...
  'ai_response_generated':    { icon: MessageSquare, color: 'var(--q-green)', label: 'AI Yanıt Üretildi' },
  ...
  'policy_blocked':            { icon: Shield, color: 'var(--q-red)', label: 'Politika Engelledi' },
  ...
  'max_messages_reached':      { icon: AlertCircle, color: 'var(--q-orange)', label: 'Mesaj Limiti' },
  'duplicate_message_dropped': { icon: CheckCircle2, color: 'var(--q-text-secondary)', label: 'Tekrar Mesaj Atıldı' },
  'prompt_version_created':    { icon: Brain, color: 'var(--q-blue)', label: 'Prompt Versiyonlandı' },
};
    ...
    <div className="space-y-6">
      ...
      {stats && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <StatCard label="Devir Teslim" value={parseInt(stats.escalations_24h) || 0} color={parseInt(stats.escalations_24h) > 0 ? 'var(--q-orange)' : undefined} />
        </div>
      )}
      ...
      <div className="rounded-xl overflow-hidden" style={{ background: 'var(--q-bg-primary)', border: '1px solid var(--q-border-default)' }}>
        <div className="px-4 py-3 flex items-center justify-between" style={{ borderBottom: '1px solid var(--q-border-default)' }}>
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold" style={{ color: 'var(--q-text-primary)' }}>Canlı Olay Akışı</h3>
          </div>
        ...
        <div className="max-h-[600px] overflow-y-auto divide-y" style={{ borderColor: 'var(--q-border-default)' }}>
          {events.length === 0 && (
            <div className="p-8 text-center">
              ...
              <p className="text-sm" style={{ color: 'var(--q-text-secondary)' }}>Henüz olay yok. AI aktiviteleri burada gerçek zamanlı görünecek.</p>
            </div>
          )}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {event.severity !== 'info' && (
                      <span className="text-[8px] font-bold uppercase px-1.5 py-0.5 rounded"
                        ...
                        {event.severity === 'error' ? 'hata' : 'uyarı'}
                      </span>
                    )}

...
const EVENT_CONFIG: Record<string, { icon: typeof Link; color: string; label: string }> = {
  ...
  tool_executed:           { icon: Wrench, color: 'var(--q-orange)', label: 'AI İşlem Yaptı' },
  tool_failed:             { icon: Wrench, color: 'var(--q-red)', label: 'AI İşlem Hatası' },
  crm_extraction_completed:{ icon: BarChart3, color: '#5AC8FA', label: 'CRM Güncellendi' },
  crm_extraction_failed:   { icon: BarChart3, color: 'var(--q-red)', label: 'CRM Güncelleme Hatası' },
  crm_raw_extracted:       { icon: BarChart3, color: 'var(--q-blue)', label: 'AI CRM Çıkarımı Yapıldı' },
  crm_resolved_for_conversation: { icon: ClipboardList, color: '#5AC8FA', label: 'Görüşme CRM Durumu Belirlendi' },
  crm_conversation_update_result: { icon: ClipboardList, color: 'var(--q-green)', label: 'CRM Eşitlemesi Tamamlandı' },
  opp_resolved_for_update: { icon: Link, color: 'var(--q-blue)', label: 'Fırsat Eşitlemesi Başladı' },
  opp_update_result:       { icon: Check, color: 'var(--q-green)', label: 'Fırsat Bilgileri Güncellendi' },
  ...
  sentiment_updated:       { icon: MessageSquare, color: 'var(--q-text-secondary)', label: 'Duygu Durumu Eşlendi' },
  ...
  working_hours_blocked:   { icon: Clock, color: 'var(--q-orange)', label: 'Mesai Dışı Bekleme' },
  max_messages_reached:    { icon: AlertTriangle, color: 'var(--q-orange)', label: 'Maks Mesaj Aşıldı' },
  duplicate_message_dropped: { icon: Copy, color: 'var(--q-text-secondary)', label: 'Çift Mesaj Ayıklandı' },
  ...
  message_status_updated:  { icon: MessageSquare, color: 'var(--q-blue)', label: 'Mesaj Durumu Güncellendi' },
  stage_changed:           { icon: BarChart3, color: 'var(--q-orange)', label: 'Aşama Değiştirildi' },
  cancellation_stage_guard:{ icon: Shield, color: 'var(--q-red)', label: 'Aşama İptal Koruması' },
  media_batch_started:     { icon: ClipboardList, color: 'var(--q-orange)', label: 'Medya Dosyaları Çözümleniyor' },
  media_batch_processed:   { icon: Check, color: 'var(--q-green)', label: 'Medya Dosyaları İşlendi' },
  media_batch_skipped_already_processed: { icon: Copy, color: 'var(--q-text-secondary)', label: 'Mevcut Medya Atlandı' },
};

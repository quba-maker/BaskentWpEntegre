...
export class HumanTonePolicy {
  ...
  public static buildDirective(input: HumanTonePolicyInput): string {
    ...
    // Core tone
    if (replyLanguage === 'tr') {
      lines.push('- İnsan gibi doğal konuş. Kalıpsal/robotik cümleler kullanma.');
      if (isHealthcare) {
      ...
      // Generic tone for other languages
    ...
    if (angryPatientMode) {
      if (replyLanguage === 'tr') {
        lines.push('- ⚠️ Kullanıcı memnuniyetsiz. Kısa özür + toparlama yap. Savunmaya geçme.');
      } else {
        lines.push('- ⚠️ User seems frustrated. Apologize briefly and recover. Don\'t be defensive.');
      }
    }

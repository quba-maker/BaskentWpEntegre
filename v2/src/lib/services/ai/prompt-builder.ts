export class PromptBuilder {
   ...
   */
  public static buildSystemPrompt(tenantPrompt: string | null, phase: string, isHumanHandover: boolean, defaultFallback?: string): string {
    if (isHumanHandover) {
      return "Kullanıcı insan temsilciye aktarıldı. Sadece kısa bir bekleme mesajı ver ve başka bir şey söyleme.";
    }

    const base = tenantPrompt || defaultFallback || "Sen kibar, profesyonel ve yardımcı bir asistan olarak hizmet veriyorsun.";

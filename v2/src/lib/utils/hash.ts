 ...
 * Avoids Node.js 'crypto' module dependencies to run safely on Edge and Client runtimes.
 */
...
/**
 * Computes a deterministic bucket (0-99) using SHA-256 hashing.
 * Guarantees uniform distribution and is fully tenant isolated.
 */
export function getRolloutBucket(input: string): number {
  const hashHex = sha256(input);
  // Parse first 8 hex characters (32-bit positive integer) and modulo 100
  const segment = parseInt(hashHex.substring(0, 8), 16);
  return segment % 100;
}

...
// ÖNEMLİ: Tüm trigger'lar handover + Telegram bildirimi tetikler.
// Ama SADECE Telegram'daki "Randevu Verildi" butonu 'appointed' yapabilir.
// Bot en fazla 'hot_lead'e kadar çıkabilir.
export function checkHandoverTriggers(userMessage, score, currentPhase) {
  const text = (userMessage || '').toLowerCase();
  
  // Yüksek skorlu lead → direkt devret (İLGİ olarak, appointed değil)
  if (score >= 50) return HANDOVER_TRIGGERS.INTEREST;
  
  // Hasta net şekilde fiyat sorarsa → İLGİ
  if (/fiyat|ücret|ne kadar|maliyet|price|cost|كم|سعر|цена|combien|wieviel/i.test(text)) {
    return HANDOVER_TRIGGERS.PRICE;
  }
  
  // === KESİN NİYET (Hasta geleceğini açıkça söylüyor) ===
  if (/geleceğim|geliyorum|gelirim|hemen gel|ayarlayın|ayarlayalım|planlayalım|onaylıyorum|kabul/i.test(text)) {
    return HANDOVER_TRIGGERS.APPOINTMENT;
  }

  // === İLGİ SİNYALİ (Hasta soruyor, ilgileniyor ama kesinleştirmedi) ===
  if (/randevu|gelebilir|appointment|booking|موعد|запись|termin|rendez|ne zaman gel|uygun.*gün|uygun.*saat|müsait/i.test(text)) {
    return HANDOVER_TRIGGERS.INTEREST;
  }

  // Hasta spesifik zaman verdiğinde veya kısa onay (trust/handover fazında) → İLGİ
  if (currentPhase === 'trust' || currentPhase === 'handover') {
    if (/olur|tamam|evet|uygun|sabah|öğle|akşam|yarın|bugün|saat\s*\d+/i.test(text) && text.length < 30) {
      return HANDOVER_TRIGGERS.INTEREST;
    }
  }
...
export async function executeHandover(phone, triggerReason, patientName, department) {
  ...
  try {
    ...
    // Aynı telefon için son 3 saatte açılmış alarm varsa tekrar açma (TEST İÇİN 1 DAKİKAYA DÜŞÜRÜLDÜ)
    const recent = await sql`SELECT id FROM alerts WHERE phone_number LIKE ${likePattern} AND created_at > NOW() - INTERVAL '1 minute'`;
    if (recent.length === 0) {
...
⚡ <i>5 dakika içinde arayın! Lütfen aşağıdaki butonlardan durumu güncelleyin:</i>`;
      
      const inlineKeyboard = {
        inline_keyboard: [
          [
            { text: "📞 Arandı - Ulaşılamadı", callback_data: `crm_callmiss_${phone}` },
            { text: "✅ Randevu Verildi", callback_data: `crm_appoint_${phone}` }
          ],
          [
            { text: "🔄 Tekrar Ara", callback_data: `crm_recall_${phone}` },

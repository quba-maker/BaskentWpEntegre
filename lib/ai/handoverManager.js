...
export function checkHandoverTriggers(userMessage, score, currentPhase) {
  ...
  if (/fiyat|ücret|ne kadar|maliyet|price|cost|كم|سعر/i.test(text)) {
    return HANDOVER_TRIGGERS.PRICE;
  }
  
  // Hasta randevu veya gelmek isterse
  if (/randevu|geleceğim|geliyorum|appointment|booking|موعد/i.test(text)) {
    return HANDOVER_TRIGGERS.APPOINTMENT;
  }

  // Hasta şikayetçi olursa (Bot anlamıyor vb.)
  if (/insan|müşteri temsilcisi|bot|anlamıyorsun|human|representative|agent/i.test(text)) {

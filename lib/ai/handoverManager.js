...
export function checkHandoverTriggers(userMessage, score, currentPhase) {
  ...
  // === TAKİP / SÜREÇ SORGUSU (Hasta "ne oldu", "aradınız mı" gibi sorular soruyorsa veya randevu/belge değişikliği istiyorsa) ===
  if (/(ne oldu|noldu|aradınız|ne zaman|tekrar arayın|değiştir|iptal|sonuç|belge|rapor)/i.test(text) && text.length < 150) {
    // Sadece "ne zaman" demek yetmeyebilir, ama bu kelimelerin hastanın kendi durumuyla ilgili sorgulama yapma ihtimali yüksek.
    // Özellikle botun "aradık ulaşamadık" notunu görüp cevap verdiği durumlarda bildirim gitmesi için bu tetikleyiciyi ekliyoruz.
...
export async function executeHandover(phone, triggerReason, patientName, department, requestedTime = '') {
  ...
  try {
    ...
    // Aynı hastaya son 1 saatte aynı tip alert gönderildiyse tekrar atma (Spam önleme)
    const recent = await sql`SELECT id FROM alerts WHERE phone_number LIKE ${likePattern} AND created_at > NOW() - INTERVAL '1 hour'`;
    if (recent.length === 0) {
      ...
      if (displayTime) {
        // Eğer ISO veya karmaşık format gelirse temizle (Örn: 2026-05-09T13:23:26-05:00)
        try {
          if (displayTime.includes('T') && displayTime.includes('-')) {
            const dt = new Date(displayTime);
            if (!isNaN(dt.getTime())) {
              displayTime = dt.toLocaleString('tr-TR', { day: '2-digit', month: 'long', year: 'numeric', hour: '2-digit', minute: '2-digit' });
            }
          }
        } catch(e) {}
      }
...
📊 Skor: ${leadScore}/100
${displayTime ? `⏰ Arama Saati Tercihi: <b>${displayTime}</b>` : ''}

...
// 🏷️ Konuşma analizi — otomatik etiket + randevu tespiti + lead scoring
export async function analyzeConversation(phone, lastUserMsg, lastBotMsg) {
  ...
  depts.forEach(d => { if (d.re.test(combined)) { tags.push(d.tag); score += d.pts; } });

  // Fiyat sorgusu → ciddi ilgi
  if (/fiyat|ücret|ne kadar|maliyet|price|cost|كم|سعر|цена/i.test(combined)) { tags.push('Fiyat Sordu'); score += 10; }

  // Gurbetçi → yüksek gelir potansiyeli
  if (/almanya|deutschland|germany|hollanda|fransa|belçika|avusturya|ingiltere|isviçre|gurbetçi|abroad|yurtdışı/i.test(combined)) { tags.push('Gurbetçi'); score += 30; }

  // Uluslararası hasta (telefon numarasından)
  if (phone && !phone.startsWith('90') && !phone.startsWith('test')) score += 20;

  // Randevu talebi → çok sıcak lead
  const aptPat = /randevu|appointment|موعد|запись|termin|rendez|müsait|ne zaman|gelebilir|gelmek istiyorum|ayarlayalım|planla|uygun|saat|tarih|geliyorum|geleceğim|evet.*randevu|istiyorum.*randevu|tamam.*gel/i;
  if (aptPat.test(combined)) { appointmentRequested = true; tags.push('Randevu İstiyor'); score += 25; }
  ...
  if (lostPat.test(combined) && !appointmentRequested) { tags.push('Kaybedildi'); score = Math.max(score - 50, 0); }
  ...
  try {
    ...
    if (appointmentRequested) {
      try {
        ...
        if (recent.length === 0) await sql`INSERT INTO events (phone_number, event_type, details, status) VALUES (${phone}, 'appointment_request', ${lastUserMsg.substring(0, 500)}, 'pending')`;
      } catch(e) {}
    }
    console.log(`🏷️ ${phone} → [${tags.join(', ')}] 📊 Skor:${score}${appointmentRequested ? ' 🔔 RANDEVU' : ''}`);
  } catch(e) { console.error('Etiket hatası:', e.message); }

  return { tags, appointmentRequested, score };
}

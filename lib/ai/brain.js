...
export async function analyzeConversation(phone, lastUserMsg, lastBotMsg) {
  ...
  if (/almanya|deutschland|germany|hollanda|fransa|belçika|avusturya|ingiltere|isviçre|gurbetçi|abroad|yurtdışı/i.test(combined)) { 
  ...
  if (aptPat.test(combined)) { appointmentRequested = true; score += 25; }
  ...
  let isLost = false;
  if (lostPat.test(combined) && !appointmentRequested) { 
    isLost = true;

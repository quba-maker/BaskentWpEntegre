...
export async function processMessage(channel, text, history = [], recipientId = null, senderPhone = null) {
  ...
  if (senderPhone) {
    ...
    // Handover Kontrolü: Sıcak sinyal var mı?
    ...
    if (nextPhase !== state.phase) {
      ...
      if (newStage) {
        try {
          ...
          // Telefon format alternatifleri
          const pLike = senderPhone.replace(/^\+/, '');
          const pAlt = pLike.startsWith('90') ? pLike.substring(2) : '90' + pLike;
          ...
          const current = await sqlSync`SELECT stage FROM leads WHERE phone_number IN (${senderPhone}, ${pLike}, ${pAlt}) ORDER BY created_at DESC LIMIT 1`;
          const currentOrder = stageOrder[current[0]?.stage] || 0;
          if (stageOrder[newStage] > currentOrder) {
            await sqlSync`UPDATE leads SET stage = ${newStage} WHERE phone_number IN (${senderPhone}, ${pLike}, ${pAlt}) AND stage NOT IN ('appointed', 'lost')`;
            await sqlSync`UPDATE conversations SET lead_stage = ${newStage} WHERE phone_number IN (${senderPhone}, ${pLike}, ${pAlt})`;
  ...
  if (senderPhone) {
    try {
      ...
      if (leads.length === 0) {
        const convData = await sql`SELECT patient_name, department, tags FROM conversations WHERE phone_number IN (${senderPhone}, ${phoneLike}, ${phoneAlt}, ${phoneWithPlus}) LIMIT 1`;
...
export async function analyzeConversation(phone, lastUserMsg, lastBotMsg) {
  ...
  try {
    ...
    if (isLost) {
      ...
      await sql`UPDATE leads SET stage = 'hot_lead' WHERE phone_number = ${phone} AND stage NOT IN ('appointed', 'lost')`;
    } else {
      // Sadece iletişim kurulduysa ve yeni formsa responded'a çek
      await sql`UPDATE leads SET stage = 'discovery' WHERE phone_number = ${phone} AND stage = 'new'`;
    }

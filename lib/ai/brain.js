...
export async function processMessage(channel, text, history = [], recipientId = null, senderPhone = null) {
  ...
  if (senderPhone) {
    ...
    if (state.phase !== 'handover') {
      ...
      if (trigger) {
        console.log(`🔥 Ekstrem Handover tetiklendi: ${trigger} (skor: ${leadScore})`);
        state.phase = 'handover'; // Yapay zekaya devretmek için fazı zorla
    ...
    if (nextPhase !== state.phase) {
      ...
      if (nextPhase === 'handover') {
        try {
          ...
          // Burada arama saatini de nota ekleyebiliriz (AI'ın sorduğu saate göre text içindedir)
          await executeHandover(senderPhone, 'appointment', patientName, conv[0]?.department, text);
...
Bu veriler hastanın formda bizzat seçtiği ve verdiği KESİN bilgilerdir.
- Hastaya nerede yaşadığını, adını veya hangi bölümle ilgilendiğini TEKRAR SORAMAZSIN!
- Bu veriler ışığında doğrudan empati kurarak konuşmayı başlat.
- "Formunuzu inceledim" deme, doğrudan bu bilgilere göre doğal bir şekilde yönlendir.

📋 Gelen Kampanya: ${lead.form_name || 'Bilinmiyor'}
👤 İsim: ${lead.patient_name || 'Bilinmiyor'}
🌍 Şehir/Ülke: ${lead.city || 'Bilinmiyor'}
🏥 Sistem Tahmini Bölüm: ${leadTags.join(', ') || 'Genel'}
${formResponsesText ? `\n📝 HASTANIN KENDİ VERDİĞİ CEVAPLAR:${formResponsesText}` : ''}
============================================================\n\n`;

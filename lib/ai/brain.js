  ...
  try {
    ...
    if (appointmentRequested) {
      try {
        ...
        if (recent.length === 0) await sql`INSERT INTO events (phone_number, event_type, details, status) VALUES (${phone}, 'appointment_request', ${userText.substring(0, 500)}, 'pending')`;
      } catch(e) {}
    }

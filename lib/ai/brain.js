...
`;
  ...
  // Kullanıcının dilini tespit et ve mesajın içine yönlendirme ekle
  const detectedLang = detectLanguageFromText(text);
  const langDirective = detectedLang ? `\n[RESPOND IN ${detectedLang.toUpperCase()} ONLY]\n` : '';
  
  const fullHistory = [...history, { role: 'user', parts: [{ text: text + langDirective }] }];

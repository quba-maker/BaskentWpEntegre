...
// 🏷️ Konuşma analizi — otomatik etiket + randevu tespiti
export async function analyzeConversation(phone, lastUserMsg, lastBotMsg) {
  ...
  // Bölüm tespiti
  if (/ortopedi|bel fıtığı|omurga|diz|kalça|kırık|eklem/i.test(combined)) tags.push('Ortopedi');
  if (/kardiyoloji|kalp|tansiyon|stent|anjio|bypass/i.test(combined)) tags.push('Kardiyoloji');
  if (/estetik|burun|yüz germe|liposuction|botox|dolgu|meme/i.test(combined)) tags.push('Estetik');
  if (/diş|implant|ortodonti|kanal tedavi|çekim|zirkonyum/i.test(combined)) tags.push('Diş');
  if (/göz|katarakt|lazer|retina|lens/i.test(combined)) tags.push('Göz');
  if (/tüp bebek|ivf|kısırlık|gebelik|doğum|kadın/i.test(combined)) tags.push('Tüp Bebek');
  if (/nakil|organ|böbrek|karaciğer|haberal/i.test(combined)) tags.push('Organ Nakli');
  if (/onkoloji|kanser|tümör|kemoterapi/i.test(combined)) tags.push('Onkoloji');
  if (/obezite|mide küçültme|sleeve|bariatrik/i.test(combined)) tags.push('Obezite');
  if (/nöroloji|beyin|baş ağrısı|epilepsi|ms/i.test(combined)) tags.push('Nöroloji');
  if (/üroloji|prostat|böbrek taşı|mesane/i.test(combined)) tags.push('Üroloji');
  if (/check.?up|genel kontrol|tarama/i.test(combined)) tags.push('Check-Up');

  // Fiyat sorgusu tespiti
  if (/fiyat|ücret|ne kadar|maliyet|price|cost|كم|سعر|цена/i.test(combined)) tags.push('Fiyat Sordu');

  // Gurbetçi tespiti (yurt dışından gelen)
  if (/almanya|deutschland|germany|hollanda|fransa|belçika|avusturya|ingiltere|isviçre|gurbetçi|abroad|yurtdışı/i.test(combined)) tags.push('Gurbetçi');

  // Randevu talebi tespiti
  const appointmentPatterns = /randevu|appointment|موعد|запись|termin|rendez|müsait|ne zaman|gelebilir|gelmek istiyorum|ayarlayalım|planla|uygun|saat|tarih|geliyorum|geleceğim|evet.*randevu|istiyorum.*randevu|tamam.*gel/i;
  if (appointmentPatterns.test(combined)) {
    appointmentRequested = true;
    tags.push('Randevu İstiyor');
  }

  // Kaybedilen hasta tespiti
  const lostPatterns = /istemiyorum|gerek yok|başka hastane|başka yere|vazgeçtim|iptal|cancel|no thanks|لا شكرا|не нужно|kein interesse|pas intéressé|almıyorum|gitmeye.*karar|başka.*(doktor|yer)/i;
  if (lostPatterns.test(combined) && !appointmentRequested) {
    tags.push('Kaybedildi');
    try {
      const { neon } = await import('@neondatabase/serverless');
      const sqlLost = neon(process.env.DATABASE_URL);
      await sqlLost`UPDATE leads SET stage = 'lost' WHERE phone_number = ${phone} AND stage NOT IN ('appointed', 'lost')`;
    } catch(e) {}
  }

  // DB güncelle
  if (tags.length > 0 || appointmentRequested) {
    try {
      const { neon } = await import('@neondatabase/serverless');
      const sql = neon(process.env.DATABASE_URL);
      
      // Mevcut etiketleri al ve birleştir
      ...
      await sql`UPDATE conversations SET tags = ${JSON.stringify(mergedTags)} WHERE phone_number = ${phone}`;

      // Randevu talebi varsa events tablosuna kaydet
      if (appointmentRequested) {
        try {
          ...
          // Aynı kişi için son 24 saatte aynı event varsa tekrar ekleme
          const recent = await sql`SELECT id FROM events WHERE phone_number = ${phone} AND event_type = 'appointment_request' AND created_at > NOW() - INTERVAL '24 hours'`;
          if (recent.length === 0) {
            await sql`INSERT INTO events (phone_number, event_type, details, status) VALUES (${phone}, 'appointment_request', ${lastUserMsg.substring(0, 500)}, 'pending')`;
          }
        } catch(e) {}
      }

      console.log(`🏷️ Otomatik etiket: ${phone} → [${tags.join(', ')}]${appointmentRequested ? ' 🔔 RANDEVU TALEBİ' : ''}`);
    } catch(e) { console.error('Etiket güncelleme hatası:', e.message); }
  }

  return { tags, appointmentRequested };
}

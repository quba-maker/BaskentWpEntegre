...
export async function processMessage(channel, text, history = [], recipientId = null, senderPhone = null) {
  ...
  // 🎯 İkna Seviyesi (Bot Yönetimi'nden ayarlanabilir)
  const aggressionLevel = await getSetting('bot_aggression_level', 'medium');
  ...
  // 🚫 Yasaklı Kelimeler (Bot Yönetimi'nden yönetilebilir)
  const bannedWordsRaw = await getSetting('bot_banned_words', '[]');
  try {
  ...
  // 📚 BİLGİ BANKASI (Bilgi Bankası Ekranından Gelen Fiyat ve Kurallar)
  const knowledgePrices = await getSetting('bot_knowledge_prices', '');
  if (knowledgePrices) {
  ...
  const knowledgeRules = await getSetting('bot_knowledge_rules', '');
  if (knowledgeRules) {

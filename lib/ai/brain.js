...
export async function processMessage(channel, text, history = [], recipientId = null, senderPhone = null) {
    ...
    if (nextPhase !== state.phase) {
      ...
      if (newStage) {
        try {
          ...
          const current = await sqlSync`SELECT stage FROM leads WHERE phone_number = ${senderPhone} ORDER BY created_at DESC LIMIT 1`;
          const currentOrder = stageOrder[current[0]?.stage] || 0;
          if (stageOrder[newStage] > currentOrder) {
            await sqlSync`UPDATE leads SET stage = ${newStage} WHERE phone_number = ${senderPhone} AND stage NOT IN ('appointed', 'lost')`;
            await sqlSync`UPDATE conversations SET lead_stage = ${newStage} WHERE phone_number = ${senderPhone}`;
  ...
  if (senderPhone) {
    try {
      const { neon } = await import('@neondatabase/serverless');
      const sql = neon(process.env.DATABASE_URL);
      const leads = await sql`SELECT patient_name, form_name, city, tags, stage, email, raw_data FROM leads WHERE phone_number = ${senderPhone} ORDER BY created_at DESC LIMIT 1`;
      if (leads.length > 0) {
        const lead = leads[0];
        let leadTags = [];
...
- Hasta tetkik/belge paylaştıysa, değerlendirme sürecini anlat
...
---\n`;
        ...
        console.log(`📋 [${channel}] Lead bilgisi + form yanıtları AI'a enjekte edildi: ${lead.form_name}`);
      }

...
export async function handleInstagramMessage(body) {
  try {
    ...
    // 2. Canlı müdahale kontrolü (Bot artık tamamen kapanmıyor, HANDOVER moduna geçiyor — WhatsApp ile aynı davranış)
    const status = await getConversationStatus(senderId);
    if (status === 'human') {
      console.log(`👤 [Instagram] İnsan müdahalesi aktif: ${senderId} - Bot sekreter modunda oyalıyor`);
    }

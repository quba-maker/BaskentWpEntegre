...
Eğer tıbbi değilse sadece "Teşekkürler, fotoğrafınız bize ulaştı." yaz.` }
    ...
    // 📛 WhatsApp profil adını kaydet (ilk mesajda)
    if (waProfileName) {
      try {
        ...
        if (!existing[0]?.patient_name || existing[0].patient_name === phone) {
          await sqlDb`UPDATE conversations SET patient_name = ${waProfileName} WHERE phone_number = ${phone}`;
          console.log(`📛 [WhatsApp] Profil adı kaydedildi: ${phone} → ${waProfileName}`);
        }
      } catch(e) {}
    }

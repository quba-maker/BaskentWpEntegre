import axios from 'axios';
import { getSetting, saveMessage, getConversationStatus, resetFollowUpCount, getConversationHistory } from '../db/index.js';
import { processMessage, analyzeConversation } from '../ai/brain.js';

// NOT: META_ACCESS_TOKEN ve PHONE_NUMBER_ID artık tenant bazlı.
// Eğer body.tenant_meta varsa o kullanılır, yoksa env fallback.

// Çalışma saatleri kontrolü
function isWorkingHours(settingsStr) {
  try {
    const h = JSON.parse(settingsStr);
    if (!h.enabled) return { working: true };
    const now = new Date();
    const tr = new Date(now.toLocaleString('en-US', { timeZone: 'Europe/Istanbul' }));
    const hour = tr.getHours();
    const min = tr.getMinutes();
    const current = hour * 60 + min;
    const [sh, sm] = h.start.split(':').map(Number);
    const [eh, em] = h.end.split(':').map(Number);
    const start = sh * 60 + sm;
    const end = eh * 60 + em;
    if (current >= start && current <= end) return { working: true };
    return { working: false, message: h.offMessage || 'Mesai saatlerimiz dışındasınız. En kısa sürede dönüş yapacağız.' };
  } catch (e) { return { working: true }; }
}

// WhatsApp üzerinden mesaj gönderme — tenant tokeniyle
export async function sendWhatsApp(phone, text, accessToken = null, phoneNumberId = null) {
  const token = accessToken || process.env.META_ACCESS_TOKEN;
  const phoneId = phoneNumberId || process.env.PHONE_NUMBER_ID;
  await axios({
    method: 'POST',
    url: `https://graph.facebook.com/v25.0/${phoneId}/messages`,
    headers: { Authorization: `Bearer ${token}` },
    data: { messaging_product: 'whatsapp', to: phone, type: 'text', text: { body: text } }
  });
}

// WhatsApp mesajını işleme (metin + sesli mesaj desteği)
export async function handleWhatsAppMessage(body) {
  try {
    // 🏭 TENANT CONTEXT — webhook router'dan enjekte edilir
    const tenantId = body.tenant_id || null;
    const tenantMeta = body.tenant_meta || {};
    const META_ACCESS_TOKEN = tenantMeta.meta_page_token || process.env.META_ACCESS_TOKEN;
    const PHONE_NUMBER_ID = tenantMeta.whatsapp_phone_id || process.env.PHONE_NUMBER_ID;

    // Kanal aktiflik durumu (tenant bazlı)
    const channelBotEnabled = await getSetting('channel_whatsapp_enabled', 'true', tenantId);

    const phone = body.entry[0].changes[0].value.contacts[0].wa_id;
    const waProfileName = body.entry[0].changes[0].value.contacts?.[0]?.profile?.name || null;
    const message = body.entry[0].changes[0].value.messages[0];

    let text = '';
    let isVoice = false;

    if (message.type === 'text') {
      text = message.text.body;
    } else if (message.type === 'audio') {
      // 🎤 Sesli mesaj — Meta'dan indir, Gemini'ye gönder
      isVoice = true;
      try {
        const mediaId = message.audio.id;
        // 1. Media URL al
        const mediaInfo = await axios.get(`https://graph.facebook.com/v25.0/${mediaId}`, {
          headers: { Authorization: `Bearer ${META_ACCESS_TOKEN}` }
        });
        const mediaUrl = mediaInfo.data.url;
        // 2. Ses dosyasını indir
        const audioRes = await axios.get(mediaUrl, {
          headers: { Authorization: `Bearer ${META_ACCESS_TOKEN}` },
          responseType: 'arraybuffer'
        });
        const audioBase64 = Buffer.from(audioRes.data).toString('base64');
        const mimeType = mediaInfo.data.mime_type || 'audio/ogg';
        
        // 3. Gemini'ye gönder — transkript al
        const GEMINI_KEY = process.env.GEMINI_API_KEY;
        const geminiRes = await axios.post(
          `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${GEMINI_KEY}`,
          {
            contents: [{
              parts: [
                { inlineData: { mimeType, data: audioBase64 } },
                { text: 'Bu sesli mesajı kelimesi kelimesine yazıya dök. Sadece transkripsiyonu yaz, başka bir şey ekleme.' }
              ]
            }]
          }
        );
        text = geminiRes.data?.candidates?.[0]?.content?.parts?.[0]?.text?.trim() || '';
        console.log(`🎤 [WhatsApp] Ses transkript (${phone}): ${text}`);
      } catch(e) {
        console.error('❌ Ses transkript hatası:', e.response?.data || e.message);
        text = '[Sesli mesaj gönderildi]';
      }
    } else if (message.type === 'image') {
      // 📷 Görüntü mesajı — MR, röntgen, tetkik sonucu olabilir
      try {
        const mediaId = message.image.id;
        const caption = message.image.caption || '';
        
        // 1. Media URL al
        const mediaInfo = await axios.get(`https://graph.facebook.com/v25.0/${mediaId}`, {
          headers: { Authorization: `Bearer ${META_ACCESS_TOKEN}` }
        });
        const mediaUrl = mediaInfo.data.url;
        
        // 2. Görüntüyü indir
        const imgRes = await axios.get(mediaUrl, {
          headers: { Authorization: `Bearer ${META_ACCESS_TOKEN}` },
          responseType: 'arraybuffer'
        });
        const imgBase64 = Buffer.from(imgRes.data).toString('base64');
        const mimeType = mediaInfo.data.mime_type || 'image/jpeg';
        
        // 3. Gemini Multimodal ile ön analiz
        const GEMINI_KEY = process.env.GEMINI_API_KEY;
        const geminiRes = await axios.post(
          `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${GEMINI_KEY}`,
          {
            contents: [{
              parts: [
                { inlineData: { mimeType, data: imgBase64 } },
                { text: `Bu bir hasta tarafından gönderilen tıbbi görüntü/belge olabilir (MR, röntgen, kan tahlili, rapor vb.).
                
GÖREV: 
1. Görüntünün ne olduğunu tespit et (MR mı, röntgen mi, kan tahlili mi, başka bir belge mi, yoksa tıbbi olmayan bir fotoğraf mı?)
2. Eğer tıbbi bir görüntüyse, GENEL bir yorum yap. KESİNLİKLE TEŞHİS KOYMA.
3. Hastayı yüreklendir ve doktor değerlendirmesine yönlendir.

YANIT FORMATI: Türkçe, sıcak, profesyonel, 2-4 cümle. 
ÖNEMLİ: Sen bir AI asistansın, doktor değilsin. "Bunu doktorumuz detaylı değerlendirecek" de.
Eğer tıbbi değilse sadece "Teşekkürler, fotoğrafınız bize ulaştı." yaz.` }
              ]
            }]
          },
          { timeout: 20000 }
        );
        
        const imgAnalysis = geminiRes.data?.candidates?.[0]?.content?.parts?.[0]?.text?.trim() || '';
        text = `[📷 Görüntü gönderildi${caption ? ': ' + caption : ''}|media_id:${mediaId}]\n\nAI Ön Değerlendirme: ${imgAnalysis}`;
        console.log(`📷 [WhatsApp] Görüntü analiz edildi (${phone}): ${imgAnalysis.substring(0, 100)}...`);
        
        // 🚨 Doktor Kontrolü için Telegram Bildirimi Gönder
        try {
          const { sendTelegramAlert } = await import('../ai/handoverManager.js');
          const cleanP = (phone || '').replace(/\D/g, '');
          const crmUrl = `https://baskent-wp-entegre.vercel.app/?phone=${cleanP}`;
          const chatUrl = `https://baskent-wp-entegre.vercel.app/?phone=${cleanP}&view=chat`;
          const formattedPhone = phone.length > 10 ? `+${phone.substring(0,2)} ${phone.substring(2,5)} ${phone.substring(5,8)} ${phone.substring(8,10)} ${phone.substring(10)}` : phone;

          let pName = '';
          let pDept = '';
          try {
            const last10 = cleanP.length > 10 ? cleanP.substring(cleanP.length - 10) : cleanP;
            const conv = await sql`SELECT patient_name, department FROM conversations WHERE phone_number LIKE ${'%' + last10 + '%'} LIMIT 1`;
            if (conv && conv.length > 0) {
              pName = conv[0].patient_name ? ` (${conv[0].patient_name})` : '';
              pDept = conv[0].department ? `\n🏥 <b>Bölüm:</b> ${conv[0].department}` : '';
            }
          } catch(e) {}

          const inlineKeyboard = {
            inline_keyboard: [
              [
                { text: "🖥️ CRM'de Görüntüle", url: crmUrl },
                { text: "💬 Yazışmaları Gör", url: chatUrl }
              ]
            ]
          };

          await sendTelegramAlert(`🚨 <b>[YENİ GÖRÜNTÜ/RAPOR GELDİ]</b>\n\n📱 <b>Hasta:</b> <a href="tel:+${phone}">${formattedPhone}</a>${pName}${pDept}\n🤖 <b>AI Analizi:</b> ${imgAnalysis.substring(0, 150)}...\n\n👉 Lütfen CRM paneline girip hastanın gönderdiği görüntüyü doktora iletin.`, inlineKeyboard);
          
          // CRM paneline bildirim (alert) düşmesi için veritabanına da yaz
          try {
            await sql`CREATE TABLE IF NOT EXISTS alerts (id SERIAL PRIMARY KEY, phone_number VARCHAR(20), alert_type VARCHAR(50), message TEXT, is_read BOOLEAN DEFAULT false, created_at TIMESTAMP DEFAULT NOW())`;
            await sql`INSERT INTO alerts (phone_number, alert_type, message) VALUES (${cleanP}, 'new_image', ${'📷 Yeni Görüntü/Rapor: ' + imgAnalysis.substring(0, 100)})`;
          } catch(dbErr) { console.error('Alert DB insert error:', dbErr.message); }

        } catch (e) {
          console.error('Görüntü Telegram bildirim hatası:', e.message);
        }
      } catch(e) {
        console.error('❌ Görüntü analiz hatası:', e.response?.data || e.message);
        text = '[📷 Görüntü gönderildi — analiz edilemedi]';
      }
    } else if (message.type === 'document') {
      // 📄 Belge (PDF rapor vb.)
      const docId = message.document?.id || '';
      text = `[📄 Belge gönderildi: ${message.document?.filename || 'dosya'}${docId ? '|media_id:' + docId : ''}]`;
    } else {
      return; // Diğer mesaj tipleri (sticker, location, vs)
    }

    if (!text) return;
    console.log(`📩 [WhatsApp] ${isVoice ? '🎤' : '💬'} (${phone}): ${text}`);
    
    // 1. Mesajı veritabanına kaydet — tenant_id ile
    await saveMessage(phone, 'in', isVoice ? `🎤 ${text}` : text, null, 'whatsapp', tenantId);
    await resetFollowUpCount(phone);

    // 📛 WhatsApp profil adını kaydet — AMA formdan gelen isim varsa ONA DOKUNMA
    if (waProfileName) {
      try {
        const { neon } = await import('@neondatabase/serverless');
        const sqlDb = neon(process.env.DATABASE_URL);
        
        // Önce formdan (leads tablosu) bir isim gelmiş mi kontrol et
        const leadName = await sqlDb`SELECT patient_name FROM leads WHERE phone_number LIKE ${'%' + phone.substring(phone.length - 10) + '%'} ORDER BY created_at DESC LIMIT 1`;
        const formName = leadName[0]?.patient_name;
        
        const existing = await sqlDb`SELECT patient_name FROM conversations WHERE phone_number = ${phone}`;
        if (!existing[0]?.patient_name || existing[0].patient_name === phone) {
          // Form adı varsa onu kullan, yoksa WhatsApp profil adını kullan
          const nameToUse = formName || waProfileName;
          await sqlDb`UPDATE conversations SET patient_name = ${nameToUse} WHERE phone_number = ${phone}`;
          console.log(`📛 [WhatsApp] İsim kaydedildi: ${phone} → ${nameToUse} (kaynak: ${formName ? 'FORM' : 'WA Profil'})`);
        }
      } catch(e) { console.error('İsim kaydetme hatası:', e.message); }
    }

    // Lead durumunu otomatik güncelle (hasta cevap verdi)
    try {
      const { neon } = await import('@neondatabase/serverless');
      const sqlDb = neon(process.env.DATABASE_URL);
      await sqlDb`UPDATE leads SET stage = 'discovery', responded_at = NOW() WHERE phone_number = ${phone} AND stage IN ('new', 'contacted')`;
    } catch(e) {}

    // 2. Canlı müdahale kontrolü — Manuel devralda bot TAMAMEN DURUR
    const status = await getConversationStatus(phone);
    if (status === 'human') {
      console.log(`🛑 [WhatsApp] Manuel devral aktif: ${phone} — Bot yanıt VERMİYOR.`);
      return; // ❌ Bot hiçbir cevap üretmez, sadece mesaj kaydedilir (yukarıda saveMessage yapıldı)
    }

    // 3. Çalışma saatleri kontrolü — tenant bazlı
    const hoursConfig = await getSetting('working_hours', '{"enabled":false}', tenantId);
    const hours = isWorkingHours(hoursConfig);
    if (!hours.working) {
      await saveMessage(phone, 'out', hours.message, 'mesai-disi', 'whatsapp', tenantId);
      try {
        await sendWhatsApp(phone, hours.message, META_ACCESS_TOKEN, PHONE_NUMBER_ID);
        console.log(`🕐 [WhatsApp] Mesai dışı yanıt gönderildi: ${phone}`);
      } catch (e) { console.error('❌ Mesaj hatası:', e.response?.data || e.message); }
      return;
    }

    // 4. Sohbet geçmişini al
    const history = await getConversationHistory(phone, 20);

    // ⛔ BOT ANA ŞARTELİ — Kanal kapalıysa mesaj kaydedilir ama bot cevap vermez
    if (channelBotEnabled === 'false') {
      console.log(`⛔ [WhatsApp] Bot devre dışı — mesaj kaydedildi, analiz yapılacak ama bot yanıt vermiyor: ${phone}`);
      analyzeConversation(phone, text, '').catch(e => console.error('Analiz hatası:', e.message));
      return;
    }

    // 5. Maks bot mesaj kontrolü — tenant bazlı
    const maxMsgsSetting = await getSetting('bot_max_messages', '8', tenantId);
    if (maxMsgsSetting !== 'unlimited') {
      try {
        const { neon } = await import('@neondatabase/serverless');
        const sqlDb = neon(process.env.DATABASE_URL);
        const botMsgCount = await sqlDb`SELECT COUNT(*) as c FROM messages WHERE phone_number = ${phone} AND direction = 'out' AND model_used IS NOT NULL AND model_used != 'panel' AND model_used != 'mesai-disi'`;
        if (parseInt(botMsgCount[0].c) >= parseInt(maxMsgsSetting)) {
          console.log(`🔄 [WhatsApp] Maks bot mesaj sınırı (${maxMsgsSetting}) aşıldı → Otomatik insana devir: ${phone}`);
          await sqlDb`UPDATE conversations SET status = 'human', temperature = 'warm' WHERE phone_number = ${phone}`;
          return;
        }
      } catch(e) { console.error('Max msg check hatası:', e.message); }
    }

    // 6. AI Beyninden cevap al — tenantId ile
    const { response, usedModel } = await processMessage('whatsapp', text, history, null, phone, tenantId);

    // 6. Cevabı kaydet ve gönder — tenant ile
    await saveMessage(phone, 'out', response, usedModel, 'whatsapp', tenantId);
    try {
      await sendWhatsApp(phone, response, META_ACCESS_TOKEN, PHONE_NUMBER_ID);
      console.log(`📤 [WhatsApp] Yanıt gönderildi: ${phone}`);
    } catch (e) { console.error('❌ Mesaj hatası:', e.response?.data || e.message); }

    // 7. Konuşma analizi — otomatik etiketleme + randevu tespiti
    analyzeConversation(phone, text, response).catch(e => console.error('Analiz hatası:', e.message));

  } catch (error) {
    console.error('❌ WhatsApp Handler Hatası:', error);
  }
}

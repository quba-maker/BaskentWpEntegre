...
export async function saveMessage(phone, dir, content, model = null, channel = 'whatsapp', tenantId = null) {
  ...
  try {
    ...
    // Konuşma geçmişini güncelle — tenant izolasyonlu
    const tenantFilter = tenantId 
      ? await sql`SELECT id FROM conversations WHERE phone_number = ${phone} AND tenant_id = ${tenantId}`
      : await sql`SELECT id FROM conversations WHERE phone_number = ${phone}`;
    
    if (tenantFilter.length > 0) {
      if (tenantId) {
        await sql`UPDATE conversations SET last_message_at = NOW(), message_count = message_count + 1, channel = ${channel} WHERE phone_number = ${phone} AND tenant_id = ${tenantId}`;
      } else {
      ...
      if (dir === 'in') {
        if (tenantId) {
          await sql`UPDATE conversations SET last_channel = ${channel} WHERE phone_number = ${phone} AND tenant_id = ${tenantId}`;
        } else {
...
export async function getConversationStatus(phone, tenantId = null) {
  if (!sql) return 'bot';
  try {
    const conv = tenantId
      ? await sql`SELECT status FROM conversations WHERE phone_number = ${phone} AND tenant_id = ${tenantId}`
      : await sql`SELECT status FROM conversations WHERE phone_number = ${phone}`;
    return conv.length > 0 ? conv[0].status : 'bot';
  } catch (e) { return 'bot'; }
}

export async function resetFollowUpCount(phone, tenantId = null) {
  if (!sql) return;
  try { 
    if (tenantId) {
      await sql`UPDATE conversations SET follow_up_count = 0 WHERE phone_number = ${phone} AND tenant_id = ${tenantId}`; 
    } else {
...
export async function getConversationState(phone, tenantId = null) {
  if (!sql) return { phase: 'greeting', temperature: 'cold' };
  try {
    // NOT: ALTER TABLE kaldırıldı — bu kolonlar setup migration'da oluşturuluyor.
    const conv = tenantId
      ? await sql`SELECT phase, temperature FROM conversations WHERE phone_number = ${phone} AND tenant_id = ${tenantId}`
      : await sql`SELECT phase, temperature FROM conversations WHERE phone_number = ${phone}`;
    if (conv.length > 0) {
      return { 
...
export async function updateConversationState(phone, phase, temperature, tenantId = null) {
  if (!sql) return;
  try {
    const tenantClause = tenantId ? ` AND tenant_id = '${tenantId}'` : '';
    if (phase && temperature) {
      if (tenantId) {
        await sql`UPDATE conversations SET phase = ${phase}, temperature = ${temperature} WHERE phone_number = ${phone} AND tenant_id = ${tenantId}`;
      } else {
        await sql`UPDATE conversations SET phase = ${phase}, temperature = ${temperature} WHERE phone_number = ${phone}`;
      }
    } else if (phase) {
      if (tenantId) {
        await sql`UPDATE conversations SET phase = ${phase} WHERE phone_number = ${phone} AND tenant_id = ${tenantId}`;
      } else {
        await sql`UPDATE conversations SET phase = ${phase} WHERE phone_number = ${phone}`;
      }
    } else if (temperature) {
      if (tenantId) {
        await sql`UPDATE conversations SET temperature = ${temperature} WHERE phone_number = ${phone} AND tenant_id = ${tenantId}`;
      } else {
...
export async function getConversationHistory(phone, limit = 20, tenantId = null) {
  if (!sql) return [];
  try {
    const prev = tenantId
      ? await sql`SELECT direction, content FROM messages WHERE phone_number = ${phone} AND tenant_id = ${tenantId} ORDER BY created_at DESC LIMIT ${limit}`
      : await sql`SELECT direction, content FROM messages WHERE phone_number = ${phone} ORDER BY created_at DESC LIMIT ${limit}`;
